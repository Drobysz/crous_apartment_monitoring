...
async def upsert_listing(session: AsyncSession, item: CrousListing) -> Listing:
    listing = await session.scalar(
        select(Listing).where(Listing.source == "crous", Listing.external_id == item.external_id)
    )
    fields = {
        key: value for key, value in item.__dict__.items() if key in Listing.__table__.columns
    }
    if listing is None:
...
async def apply_snapshot(
    session: AsyncSession, search: Search, items: list[CrousListing]
) -> set[int]:
    ...
    prior = {
        row.listing_id: row
        for row in (
            await session.scalars(select(SearchListing).where(SearchListing.search_id == search.id))
        ).all()
    }
