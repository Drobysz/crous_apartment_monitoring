...
def main_menu(
    ...
    *,
    ...
    rows = [[button("housing-monitoring", "housing"), button("restaurant", "restaurant")], [button("subscription", "subscription"), button("language", "language")]]
    if show_test_reset:
        rows.append([button("test-reset-subscription", "test-reset")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def housing_menu(language: str, version: int, *, monitoring_enabled: bool, translator: Translator = i18n) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text=translator.text(language, "view-available"), callback_data=NavCallback(action="list", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "filters"), callback_data=NavCallback(action="filters", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "check-now"), callback_data=NavCallback(action="check-now", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "disable-monitoring" if monitoring_enabled else "enable-monitoring"), callback_data=NavCallback(action="monitor-toggle", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "set-location"), callback_data=NavCallback(action="location", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "back"), callback_data=NavCallback(action="menu", version=version).pack())],
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows)


def restaurant_menu(language: str, version: int, translator: Translator = i18n) -> InlineKeyboardMarkup:
    rows = [
        [InlineKeyboardButton(text=translator.text(language, "restaurant-select"), callback_data=NavCallback(action="restaurant-select", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "restaurant-info"), callback_data=NavCallback(action="restaurant-info", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "restaurant-menu"), callback_data=NavCallback(action="restaurant-menu", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "restaurant-delivery"), callback_data=NavCallback(action="restaurant-delivery", version=version).pack())],
        [InlineKeyboardButton(text=translator.text(language, "back"), callback_data=NavCallback(action="menu", version=version).pack())],
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows)
