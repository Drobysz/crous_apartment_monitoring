...
from app.bot.callbacks import FavoriteCallback, NavCallback
...
from app.bot.states import FilterFlow, LocationFlow, ReportFlow, RestaurantFlow
...
from app.favourites.service import add_favorite as add_housing_favorite
from app.favourites.service import favorites as housing_favorites
from app.favourites.service import remove_favorite as remove_housing_favorite
...
def build_router(
    ...
    async def run_search_sync(search_id: int, bot: Bot, correlation_id: str) -> str:
        ...
        try:
            async with SearchLock(
                redis, search_id, settings.monitoring_lock_ttl_seconds
            ) as acquired:
                if not acquired:
                    return "busy"
                return await synchronize_search(
                    session_factory, bot, crous, search_id, correlation_id=correlation_id
                )
        finally:
            await redis.aclose()
    ...
    async def user_for(message: Message | CallbackQuery, session: AsyncSession) -> User:
        ...
        return await get_or_create_user(
            session, source.id, chat_id, source.language_code, source.username
        )

    async def current(callback: CallbackQuery, data: NavCallback, user: User) -> bool:
        if not callback.message or not nav.is_current_callback(
            user, callback.message.chat.id, callback.message.message_id, data.version
        ):
    ...
    async def render_location(bot: Bot, session: AsyncSession, user: User) -> None:
        ...
        await nav.render_text_screen(
            ...
            session,
            user,
            i18n.text(user.language, "choose-location"),
            location_menu(user.language, version),
            "location",
        )
    ...
    async def render_filters(bot: Bot, session: AsyncSession, user: User) -> None:
        ...
        if search is None:
            await nav.render_text_screen(
                ...
                session,
                user,
                i18n.text(language, "no-search"),
                back(language, version),
                "filters",
            )
            return
        if not await can_use_advanced_filters(session, user):
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text=i18n.text(language, "subscription"),
                            callback_data=NavCallback(
                                action="subscription", version=version
                            ).pack(),
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text=i18n.text(language, "back"),
            ...
            await nav.render_text_screen(
                ...
                session,
                user,
                i18n.text(language, "filters-requires-premium"),
                keyboard,
                "filters-locked",
            )
            return
        ...
        format_value = (
            i18n.text(language, "not-set")
            if not format_code
            else i18n.text(language, f"format-{format_code}")
        )
        text = i18n.text(
            language, "filters-page", price=price, surface=surface, format=format_value
        )
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=i18n.text(language, "filter-price", value=price),
                        callback_data=NavCallback(action="filter-price", version=version).pack(),
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=i18n.text(language, "filter-surface", value=surface),
                        callback_data=NavCallback(action="filter-surface", version=version).pack(),
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=i18n.text(language, "filter-format", value=format_value),
                        callback_data=NavCallback(action="filter-format", version=version).pack(),
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=i18n.text(language, "clear-filters"),
                        callback_data=NavCallback(action="filter-clear", version=version).pack(),
                    )
                ],
                [
                    InlineKeyboardButton(
    ...
    async def render_subscription(bot: Bot, session: AsyncSession, user: User) -> None:
        ...
        language, version = user.language, user.active_navigation_version + 1
        ends = (
            effective.entitlement.ends_at.strftime("%d/%m/%Y")
            if effective.entitlement and effective.entitlement.ends_at
            else i18n.text(language, "unlimited")
        )
        starts = (
            effective.entitlement.starts_at.strftime("%d/%m/%Y") if effective.entitlement else "—"
        )
        interval = i18n.text(
            language, "interval-minutes", count=max(1, plan_interval(effective.plan) // 60)
        )
        features = ", ".join(
            i18n.text(language, f"feature-{feature}") for feature in plan_features(effective.plan)
        ) or i18n.text(language, "feature-basic")
        text = i18n.text(
            language,
            "subscription-page",
            plan=i18n.text(language, f"plan-{effective.plan.code}"),
            starts=starts,
            ends=ends,
            interval=interval,
            features=features,
        ...
        if pending and pending.entitlement:
            text += "\n\n" + i18n.text(
                ...
                plan=i18n.text(language, f"plan-{pending.plan.code}"),
                starts=pending.entitlement.starts_at.strftime("%d/%m/%Y"),
            )
        ...
        if user.trial_used_at is None:
            rows.append(
                [
                    InlineKeyboardButton(
                        text=i18n.text(language, "start-trial"),
                        callback_data=NavCallback(action="trial", version=version).pack(),
                    )
                ]
            )
                ...
                [
                    InlineKeyboardButton(
                        text=i18n.text(
                            language,
                            "plan-button",
                            plan=i18n.text(language, f"plan-{plan.code}"),
                            price=price,
                        ),
                        callback_data=NavCallback(
                            action="subplan", entity=index, version=version
                        ).pack(),
                    )
                ]
            )
        ...
        await nav.render_text_screen(
            bot, session, user, text, InlineKeyboardMarkup(inline_keyboard=rows), "subscription"
        )
    ...
    async def render_plan_detail(bot: Bot, session: AsyncSession, user: User, index: int) -> None:
        ...
        if index < 0 or index >= len(plans):
            await render_subscription(bot, session, user)
            return
        plan, language, version = plans[index], user.language, user.active_navigation_version + 1
        validity = (
            i18n.text(language, "unlimited")
            if plan.code == "lifetime"
            else i18n.text(language, "plan-validity", code=plan.code)
        )
        text = i18n.text(
            language,
            "plan-detail",
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=i18n.text(language, "buy"),
                        callback_data=NavCallback(
                            action="buy", entity=index, version=version
                        ).pack(),
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=i18n.text(language, "back"),
                        callback_data=NavCallback(action="subscription", version=version).pack(),
                    )
                ],
            ]
        )
    ...
    async def render_monitoring(bot: Bot, session: AsyncSession, user: User) -> None:
        search, language, version = (
            await latest_search(session, user),
            user.language,
            user.active_navigation_version + 1,
        )
        ...
        await nav.render_text_screen(
            ...
            i18n.text(
                language,
                "monitoring-page",
                status=i18n.text(language, "enabled" if enabled else "disabled"),
            ),
            keyboard,
            "monitoring",
        )
    ...
    async def render_housing(bot: Bot, session: AsyncSession, user: User) -> None:
        ...
        await nav.render_text_screen(
            ...
            session,
            user,
            i18n.text(user.language, "housing-monitoring"),
            housing_menu(
                user.language, version, monitoring_enabled=bool(search and search.is_active)
            ),
            "housing",
        )

    def listing_card(listing: Listing) -> CrousListing:
        return CrousListing(
            ...
            surface_min=listing.surface_min,
            surface_max=listing.surface_max,
            surface_original=listing.surface_original,
            occupancy_type=listing.occupancy_type,
            bed_information=listing.bed_information,
            sanitary_information=listing.sanitary_information,
            kitchen_information=listing.kitchen_information,
            equipment=listing.equipment,
            primary_image_url=listing.primary_image_url,
            raw_payload=listing.raw_payload,
        )

    async def render_housing_favorites(bot: Bot, session: AsyncSession, user: User) -> None:
        values, version = await housing_favorites(session, user), user.active_navigation_version + 1
        if not values:
            await nav.render_text_screen(
                bot,
                session,
                user,
                i18n.text(user.language, "housing-no-favorites"),
                back(user.language, version, "housing"),
                "housing-favorites",
            )
            return
        await nav.render_text_screen(
            ...
            session,
            user,
            i18n.text(user.language, "housing-favorites"),
            back(user.language, version, "housing"),
            "housing-favorites",
        )
        for listing in values:
            await send_accommodation_card(
                bot,
                ...
                is_favorite=True,
                favourites_view=True,
            )

    async def render_reports(bot: Bot, session: AsyncSession, user: User) -> None:
        version = user.active_navigation_version + 1
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=i18n.text(user.language, "my-reports"),
                        callback_data=NavCallback(action="reports-list", version=version).pack(),
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=i18n.text(user.language, "write-report"),
                        callback_data=NavCallback(action="reports-write", version=version).pack(),
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=i18n.text(user.language, "back"),
                        callback_data=NavCallback(action="menu", version=version).pack(),
                    )
                ],
            ]
        ...
        await nav.render_text_screen(
            bot, session, user, i18n.text(user.language, "reports"), keyboard, "reports"
        )

    async def render_user_reports(bot: Bot, session: AsyncSession, user: User) -> None:
        values = await user_reports(session, user)
        version = user.active_navigation_version + 1
        if not values:
            await nav.render_text_screen(
                bot,
                session,
                user,
                i18n.text(user.language, "reports-empty"),
                back(user.language, version, "reports"),
                "reports-list",
            )
            return
        cards = []
    ...
    async def render_favorites(bot: Bot, session: AsyncSession, user: User) -> None:
        ...
        rows = [
            [
                InlineKeyboardButton(
                    text=value.name,
                    callback_data=NavCallback(
                        action="restaurant-manage", entity=value.id, version=version
                    ).pack(),
                )
            ]
            for value in values
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text=i18n.text(user.language, "back"),
                    callback_data=NavCallback(action="restaurant", version=version).pack(),
                )
            ]
        )
        await nav.render_text_screen(
            ...
            user,
            i18n.text(
                user.language, "restaurant-favorites" if values else "restaurant-no-favorites"
            ),
    ...
    async def language_command(message: Message) -> None:
        async with session_factory() as session:
            ...
            await nav.render_text_screen(
                ...
                session,
                user,
                i18n.text(user.language, "language"),
                language_menu(user.language, version),
                "language",
            )
            await session.commit()
    ...
    async def informational_command(message: Message) -> None:
        async with session_factory() as session:
            ...
            await nav.render_text_screen(
                ...
                session,
                user,
                i18n.text(user.language, key),
                back(user.language, version),
                key,
            )
            await session.commit()
        ...
        async with session_factory() as session:
            ...
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text=i18n.text(user.language, "delete-yes"),
                            callback_data=NavCallback(action="delete-yes", version=version).pack(),
                        ),
                        InlineKeyboardButton(
                            text=i18n.text(user.language, "delete-no"),
                            callback_data=NavCallback(action="menu", version=version).pack(),
                        ),
                    ]
                ]
            ...
            await nav.render_text_screen(
                ...
                session,
                user,
                i18n.text(user.language, "delete-confirm"),
                keyboard,
                "delete",
            )
            await session.commit()
    ...
    async def navigation(
        ...
        async with session_factory() as session:
            ...
            if action == "menu":
                ...
                await state.clear()
                await render_housing(callback.bot, session, user)
            elif action == "housing-favorites":
                await state.clear()
                await render_housing_favorites(callback.bot, session, user)
            elif action == "reports":
                await state.clear()
                ...
                await nav.render_text_screen(
                    ...
                    session,
                    user,
                    i18n.text(user.language, "report-prompt"),
                    back(user.language, version, "reports"),
                    "reports-write",
                )
                ...
                keyboard = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "restaurant-near"),
                                callback_data=NavCallback(
                                    action="restaurant-near", version=version
                                ).pack(),
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "restaurant-search-city"),
                                callback_data=NavCallback(
                                    action="restaurant-search-city", version=version
                                ).pack(),
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "restaurant-favorites"),
                                callback_data=NavCallback(
                                    action="restaurant-favorites", version=version
                                ).pack(),
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "back"),
                                callback_data=NavCallback(
                                    action="restaurant", version=version
                                ).pack(),
                            )
                        ],
                    ]
                ...
                await nav.render_text_screen(
                    ...
                    session,
                    user,
                    i18n.text(user.language, "restaurant-select-prompt"),
                    keyboard,
                    "restaurant-select",
                )
                ...
                await nav.render_text_screen(
                    ...
                    session,
                    user,
                    i18n.text(user.language, "restaurant-search-prompt"),
                    back(user.language, version, "restaurant-select"),
                    "restaurant-city",
                )
            elif action == "restaurant-near":
                await state.set_state(RestaurantFlow.location_input)
                await callback.message.answer(
                    i18n.text(user.language, "restaurant-location-prompt"),
                    reply_markup=ReplyKeyboardMarkup(
                        keyboard=[
                            [
                                KeyboardButton(
                                    text=i18n.text(user.language, "send-location"),
                ...
                if callback_data.entity < len(rows) and isinstance(
                    ...
                    try:
                        ...
                        await callback.message.answer(
                            i18n.text(user.language, "restaurant-duplicate")
                        )
                ...
                if favorite is None:
                    ...
                    try:
                        if action == "restaurant-info":
                            ...
                            await callback.message.answer(
                                restaurant_information(restaurant, user.language)
                                if restaurant
                                else i18n.text(user.language, "restaurant-menu-unavailable")
                            )
                        else:
                            version = user.active_navigation_version + 1
                            keyboard = InlineKeyboardMarkup(
                                inline_keyboard=[
                                    [
                                        InlineKeyboardButton(
                                            text=i18n.text(user.language, "menu-today"),
                                            callback_data=NavCallback(
                                                action="restaurant-menu-day",
                                                entity=0,
                                                version=version,
                                            ).pack(),
                                        )
                                    ...
                                    [
                                        InlineKeyboardButton(
                                            text=i18n.text(user.language, "menu-tomorrow"),
                                            callback_data=NavCallback(
                                                action="restaurant-menu-day",
                                                entity=1,
                                                version=version,
                                    ...
                                    [
                                        InlineKeyboardButton(
                                            text=i18n.text(user.language, "menu-next-seven-days"),
                                            callback_data=NavCallback(
                                                action="restaurant-menu-day",
                                                entity=7,
                                                version=version,
                            ...
                            await nav.render_text_screen(
                                ...
                                session,
                                user,
                                i18n.text(user.language, "restaurant-menu"),
                                keyboard,
                                "restaurant-menu",
                            )
                ...
                if favorite:
                    ...
                    try:
                        for offset in (
                            range(7) if callback_data.entity == 7 else (callback_data.entity,)
                        ):
                            await callback.message.answer(
                                menu_text(
                                    await client.menu(
                        ...
                        [
                            InlineKeyboardButton(
                                text=i18n.text(
                                    ...
                                    else "delivery-enable",
                                ),
                                callback_data=NavCallback(
                                    action="restaurant-delivery-toggle", version=version
                                ).pack(),
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text="08:00",
                                callback_data=NavCallback(
                                    action="restaurant-delivery-time", entity=8, version=version
                                ).pack(),
                            ),
                            InlineKeyboardButton(
                                text="12:00",
                                callback_data=NavCallback(
                                    action="restaurant-delivery-time", entity=12, version=version
                                ).pack(),
                            ),
                            InlineKeyboardButton(
                                text="18:00",
                                callback_data=NavCallback(
                                    action="restaurant-delivery-time", entity=18, version=version
                                ).pack(),
                            ),
                        ],
                        [
                ...
                await nav.render_text_screen(
                    ...
                    session,
                    user,
                    i18n.text(user.language, "delivery-time"),
                    keyboard,
                    "restaurant-delivery",
                )
                ...
                await callback.message.answer(
                    i18n.text(
                        ...
                        if subscription.delivery_enabled
                        else "restaurant-delivery-disabled",
                    )
                )
                await render_restaurant(callback.bot, session, user)
            elif action == "restaurant-delivery-time":
                subscription = await get_restaurant_subscription(session, user)
                subscription.delivery_time = subscription.delivery_time.replace(
                    hour=callback_data.entity, minute=0
                )
                await callback.message.answer(
                    i18n.text(
                        user.language,
                        "restaurant-delivery-time-saved",
                        value=subscription.delivery_time.strftime("%H:%M"),
                    )
                )
                await render_restaurant(callback.bot, session, user)
                ...
                await remove_favorite(session, user, callback_data.entity)
                await render_favorites(callback.bot, session, user)
                ...
                if not await can_use_advanced_filters(session, user):
                    ...
                    await nav.render_text_screen(
                        ...
                        session,
                        user,
                        i18n.text(user.language, "price-input"),
                ...
                if not await can_use_advanced_filters(session, user):
                    ...
                    await nav.render_text_screen(
                        ...
                        session,
                        user,
                        i18n.text(user.language, "surface-input"),
                        back(user.language, version, "filters"),
                ...
                keyboard = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "format-individual"),
                                callback_data=NavCallback(
                                    action="format", entity=1, version=version
                                ).pack(),
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "format-colocation"),
                                callback_data=NavCallback(
                                    action="format", entity=2, version=version
                                ).pack(),
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "format-any"),
                                callback_data=NavCallback(
                                    action="format", entity=0, version=version
                                ).pack(),
                            )
                        ],
                        [
                ...
                await nav.render_text_screen(
                    ...
                    session,
                    user,
                    i18n.text(user.language, "format-input"),
                    keyboard,
                    "filter-format",
                )
            elif action == "format":
                ...
                if search and await can_use_advanced_filters(session, user):
                    search.accommodation_format = (
                        (None, "individual", "colocation")[callback_data.entity]
                        if callback_data.entity < 3
                        else None
                    )
                await render_filters(callback.bot, session, user)
                ...
                keyboard = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "confirm"),
                                callback_data=NavCallback(
                                    action="filter-clear-confirm", version=version
                                ).pack(),
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "cancel"),
                                callback_data=NavCallback(action="filters", version=version).pack(),
                            )
                        ],
                    ]
                ...
                if callback_data.entity >= len(plans):
                    ...
                    try:
                        checkout_url = await create_checkout_session(
                            session, user, plans[callback_data.entity].code
                        )
                    except StripeError:
                        version = user.active_navigation_version + 1
                        await nav.render_text_screen(
                            ...
                            session,
                            user,
                            i18n.text(user.language, "payment-unavailable"),
                            back(user.language, version, "subscription"),
                            "payment",
                        )
                    else:
                        version = user.active_navigation_version + 1
                        keyboard = InlineKeyboardMarkup(
                            inline_keyboard=[
                                [
                                    InlineKeyboardButton(
                                        text=i18n.text(user.language, "open-payment"),
                                        url=checkout_url,
                                    )
                                ],
                                [
                        ...
                        await nav.render_text_screen(
                            ...
                            session,
                            user,
                            i18n.text(user.language, "payment-page"),
                            keyboard,
                            "payment",
                        )
                ...
                if search is None:
                    ...
                    await main_screen(
                        ...
                        nav,
                        notice=i18n.text(
                            user.language, "resumed" if search.is_active else "paused"
                        ),
                    )
                ...
                keyboard = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "disable-monitoring"),
                                callback_data=NavCallback(
                                    action="monitor-disable", version=version
                                ).pack(),
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text=i18n.text(user.language, "back"),
                                callback_data=NavCallback(
                                    action="monitoring", version=version
                                ).pack(),
                            )
                        ],
                    ]
                ...
                await nav.render_text_screen(
                    ...
                    session,
                    user,
                    i18n.text(user.language, "disable-monitoring-confirm"),
                    keyboard,
                    "monitoring-confirm",
                )
                ...
                await nav.render_text_screen(
                    ...
                    session,
                    user,
                    i18n.text(user.language, "enter-city"),
                ...
                if callback_data.entity < len(places):
                    ...
                    await nav.render_text_screen(
                        ...
                        radius_menu(
                            user.language, version, include_entire_city=include_entire_city
                        ),
                        "radius",
                    )
            elif action == "geo":
                ...
                await callback.message.answer(
                    i18n.text(user.language, "send-location"), reply_markup=location_keyboard
                )  # temporary, allowed only here
            elif action == "radius":
                ...
                if not place:
                    ...
                    try:
                        ...
                        logger.warning(
                            ...
                            reason=str(error),
                            raw_bounds={
                                key: place.get(key) for key in ("west", "south", "east", "north")
                            },
                        )
                        await state.clear()
                        await render_location(callback.bot, session, user)
                        await callback.message.answer(
                            i18n.text(user.language, "invalid-location-area")
                        )
                    else:
                        await state.clear()
                        await main_screen(
                ...
                if await can_check_now(session, user):
                    ...
                    await nav.render_text_screen(
                        ...
                        session,
                        user,
                        i18n.text(user.language, "check-now-requires-premium"),
                        keyboard,
                        "check-now-locked",
                    )
                ...
                if get_settings().is_developer(user.telegram_user_id):
                    ...
                    await main_screen(
                        ...
                        nav,
                        notice=i18n.text(
                            user.language, "test-reset-done" if removed else "test-reset-none"
                        ),
                    )
    ...
    async def favorite_callback(callback: CallbackQuery, callback_data: FavoriteCallback) -> None:
        await callback.answer()
        async with session_factory() as session:
            user = await user_for(callback, session)
            listing = await session.get(Listing, callback_data.listing_id)
            if listing is None:
                await callback.answer(i18n.text(user.language, "outdated"), show_alert=True)
                return
            if callback_data.saved:
                await remove_housing_favorite(session, user, listing.id)
            else:
                await add_housing_favorite(session, user, listing.id)
            await session.commit()
            if not callback.message:
                return
            if callback_data.saved and callback_data.favourites_view:
                try:
                ...
                return
            try:
                from app.notifications.renderer import card_keyboard
                ...
                await callback.message.edit_reply_markup(
                    reply_markup=card_keyboard(
                        ...
                        user.language,
                        listing_id=listing.id,
                        is_favorite=not bool(callback_data.saved),
                        favourites_view=bool(callback_data.favourites_view),
                    )
                )
            except TelegramBadRequest:
                return

    @router.message(ReportFlow.text_input)
    async def report_text_input(message: Message, state: FSMContext) -> None:
        if not message.text:
            async with session_factory() as session:
                user = await user_for(message, session)
                await message.answer(i18n.text(user.language, "report-text-only"))
                await session.commit()
            return
        async with session_factory() as session:
            user = await user_for(message, session)
            try:
                ...
                await render_reports(message.bot, session, user)
            await session.commit()
    ...
    async def present_restaurants(
        message: Message, session: AsyncSession, user: User, state: FSMContext, rows: list[object]
    ) -> None:
        ...
        await nav.render_text_screen(
            ...
            session,
            user,
            i18n.text(user.language, "restaurant-select"),
            keyboard,
            "restaurant-results",
        )
    ...
    async def restaurant_city_input(message: Message, state: FSMContext) -> None:
        ...
        async with session_factory() as session:
            ...
            try:
                ...
                await present_restaurants(
                    message, session, user, state, [value.metadata for value in values]
                )
            except RestaurantUnavailable:
    ...
    async def present_places(
        bot: Bot, session: AsyncSession, user: User, state: FSMContext, places: list[GeocodedPlace]
    ) -> None:
        ...
        rows = [
            [
                InlineKeyboardButton(
                    ...
                    callback_data=NavCallback(action="place", entity=index, version=version).pack(),
                )
            ]
            for index, place in enumerate(places[:5])
        ]
        rows.append(
            [
                InlineKeyboardButton(
    ...
    async def save_search(
        session: AsyncSession, user: User, raw: dict[str, object], radius: int
    ) -> Search:
        if radius:
    ...
    async def send_current_listings(
    ...
    async def send_stored_listings(
        callback: CallbackQuery, session: AsyncSession, user: User
    ) -> None:
        search = await latest_search(session, user)
        if search is None:
        ...
        await nav.render_text_screen(
            ...
            session,
            user,
            i18n.text(user.language, "list-loading"),
            back(user.language, version),
            "list",
        )
        rows = await session.scalars(
            ...
            .where(
                SearchListing.search_id == search.id, SearchListing.is_currently_available.is_(True)
            )
            .order_by(Listing.last_seen_at.desc())
        )
        listings = list(rows)
        saved_ids = {listing.id for listing in await housing_favorites(session, user)}
        for listing in listings:
            await send_accommodation_card(
                ...
                listing.first_seen_at,
                listing_id=listing.id,
                is_favorite=listing.id in saved_ids,
            )
        version = user.active_navigation_version + 1
        await nav.render_text_screen(
            callback.bot,
            session,
            user,
            i18n.text(user.language, "list-result", count=len(listings)),
            back(user.language, version),
            "list",
        )
