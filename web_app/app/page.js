...
export function ReferralDetails({ id, locale, t, onBack }) {
  const [period, setPeriod] = useState("month"); const [page, setPage] = useState(1);
  const details = useRemote((signal) => request(`/referrals/${id}`, { signal }), [id]);
  const stats = useRemote((signal) => request(`/referrals/${id}/stats?period=${period}`, { signal }), [id, period]);
  const purchases = useRemote((signal) => request(`/referrals/${id}/purchases?page=${page}`, { signal }), [id, page]);
