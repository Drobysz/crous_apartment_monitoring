...
function ReportDetails({ id, onClose, locale, t }) {
  const remote = useRemote((signal) => request(`/reports/${id}`, { signal }), [id]);
  const item = remote.data;
  const username = item?.username;
  return <Dialog title={t("report")} onClose={onClose} t={t}><Status {...remote} empty={false} t={t}>{item && <dl className="details"><div><dt>{t("submitted")}</dt><dd>{formatDate(item.created_at, locale)}</dd></div><div><dt>{t("user")}</dt><dd>{username ? <a className="telegram-link" href={`https://t.me/${username.replace(/^@/, "")}`} target="_blank" rel="noreferrer">{username}</a> : t("unknown")}</dd></div><div><dt>{t("platform")}</dt><dd>{item.platform || t("unknown")}</dd></div><div className="details-wide"><dt>{t("fullReport")}</dt><dd className="report-text">{item.text}</dd></div></dl>}</Status></Dialog>;
}

function Reports({ t, locale }) {
  const [query, setQuery] = useState(""); const [page, setPage] = useState(1); const [selected, setSelected] = useState(null);
  useEffect(() => { const id = setTimeout(() => setPage(1), 250); return () => clearTimeout(id); }, [query]);
  const remote = useRemote((signal) => request(`/reports?q=${encodeURIComponent(query)}&page=${page}`, { signal }), [query, page]);
  return <section className="view"><header className="view-header"><div><h1>{t("reports")}</h1><p>{remote.data?.meta?.total ?? 0} {t("results")}</p></div></header><SearchField value={query} onChange={setQuery} t={t} /><Status {...remote} empty={!remote.data?.items?.length} t={t}><div className="table-scroll"><table><thead><tr><th>{t("username")}</th><th>{t("preview")}</th><th>{t("submitted")}</th></tr></thead><tbody>{remote.data?.items.map((item) => <tr key={item.id} className="report-row" tabIndex={0} onClick={() => setSelected(item.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(item.id); } }}><td>{item.username ? <a className="telegram-link" href={`https://t.me/${item.username.replace(/^@/, "")}`} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>{item.username}</a> : t("unknown")}</td><td>{item.preview}</td><td>{formatDate(item.created_at, locale)}</td></tr>)}</tbody></table></div><Pagination meta={remote.data?.meta} onPage={setPage} t={t} /></Status>{selected && <ReportDetails id={selected} onClose={() => setSelected(null)} locale={locale} t={t} />}</section>;
}

function ReferralForm({ onClose, onCreated, t }) {
  const [form, setForm] = useState({ owner_telegram_username: "", referral_code: "" });
  const [error, setError] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const ownerId = useId(); const codeId = useId();
  async function submit(event) {
    event.preventDefault(); setSubmitting(true); setError("");
    try {
      await request("/referrals", { method: "POST", body: JSON.stringify(form), headers: { Origin: window.location.origin } });
      onCreated(); onClose();
    } catch (requestError) { setError(typeof requestError.message === "string" ? requestError.message : t("error")); }
    finally { setSubmitting(false); }
  }
  return <Dialog title={t("createReferral")} onClose={submitting ? () => {} : onClose} t={t}><form className="dialog-form" onSubmit={submit}><label htmlFor={ownerId}>{t("ownerUsername")}</label><input id={ownerId} value={form.owner_telegram_username} onChange={(event) => setForm((value) => ({ ...value, owner_telegram_username: event.target.value }))} required dir="ltr" /><label htmlFor={codeId}>{t("referralCode")}</label><input id={codeId} value={form.referral_code} onChange={(event) => setForm((value) => ({ ...value, referral_code: event.target.value }))} required dir="ltr" />{error && <p className="form-error" role="alert">{error}</p>}<footer><button className="button" type="button" onClick={onClose} disabled={submitting}>{t("cancel")}</button><button className="button button-primary" type="submit" disabled={submitting}>{t("save")}</button></footer></form></Dialog>;
}

function Referrals({ t, locale, profile }) {
  const remote = useRemote((signal) => request("/referrals", { signal }), []);
  const [adding, setAdding] = useState(false); const [selected, setSelected] = useState(null); const [deleting, setDeleting] = useState(null);
  const [notice, setNotice] = useState("");
  async function copy(item, event) {
    event.stopPropagation();
    try { await navigator.clipboard.writeText(item.deep_link); setNotice(t("referralLinkCopied")); }
    catch { setNotice(t("copyFailed")); }
  }
  async function toggle(item) {
    await request(`/referrals/${item.id}`, { method: "PATCH", body: JSON.stringify({ is_active: !item.is_active }), headers: { Origin: window.location.origin } });
    remote.reload();
  }
  async function remove() {
    if (!deleting) return;
    await request(`/referrals/${deleting.id}`, { method: "DELETE", headers: { Origin: window.location.origin } });
    setDeleting(null); remote.reload();
  }
  function open(item) { window.history.pushState({}, "", `/admin/referrals/${item.id}`); setSelected(item.id); }
  if (selected) return <ReferralDetails id={selected} locale={locale} t={t} onBack={() => { window.history.pushState({}, "", "/admin/referrals"); setSelected(null); remote.reload(); }} />;
  return <section className="view"><header className="view-header"><div><h1>{t("referrals")}</h1><p>{remote.data?.items?.length ?? 0} {t("results")}</p></div><button className="button button-primary" type="button" onClick={() => setAdding(true)}>{t("createReferral")}</button></header>{notice && <p className="field-help" role="status">{notice}</p>}<Status {...remote} empty={!remote.data?.items?.length} t={t}><div className="table-scroll"><table><thead><tr><th>{t("referralCode")}</th><th>{t("ownerUsername")}</th><th>{t("status")}</th><th>{t("available")}</th><th>{t("earned")}</th><th>{t("paid")}</th><th><span className="sr-only">{t("copyLink")}</span></th></tr></thead><tbody>{remote.data?.items.map((item) => <tr key={item.id} className="report-row" tabIndex={0} onClick={() => open(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); open(item); } }}><td dir="ltr">{item.referral_code}</td><td dir="ltr">{item.owner_telegram_username}</td><td><button className="text-button" type="button" onClick={(event) => { event.stopPropagation(); toggle(item); }}>{item.is_active ? t("active") : t("inactive")}</button></td><td>{formatMoney(item.available_cents, locale)}</td><td>{formatMoney(item.lifetime_earned_cents, locale)}</td><td>{formatMoney(item.paid_cents, locale)}</td><td><button className="row-action" type="button" onClick={(event) => copy(item, event)}>{t("copyLink")}</button>{profile.role === "superadmin" && <button className="row-action" type="button" onClick={(event) => { event.stopPropagation(); setDeleting(item); }}>{t("delete")}</button>}</td></tr>)}</tbody></table></div></Status>{adding && <ReferralForm onClose={() => setAdding(false)} onCreated={remote.reload} t={t} />}{deleting && <Dialog title={t("deleteReferral")} onClose={() => setDeleting(null)} t={t}><p className="dialog-copy">{t("deleteReferralConfirm")}</p><footer className="dialog-footer"><button className="button" type="button" onClick={() => setDeleting(null)}>{t("cancel")}</button><button className="button button-primary" type="button" onClick={remove}>{t("delete")}</button></footer></Dialog>}</section>;
}

export function ReferralDetails({ id, locale, t, onBack, loadDetail, loadStats, loadPurchases }) {
  const [period, setPeriod] = useState("month"); const [page, setPage] = useState(1);
  const details = useRemote((signal) => loadDetail ? loadDetail(signal) : request(`/referrals/${id}`, { signal }), [id, loadDetail]);
  const stats = useRemote((signal) => loadStats ? loadStats(period, signal) : request(`/referrals/${id}/stats?period=${period}`, { signal }), [id, period, loadStats]);
  const purchases = useRemote((signal) => loadPurchases ? loadPurchases(page, signal) : request(`/referrals/${id}/purchases?page=${page}`, { signal }), [id, page, loadPurchases]);
  const data = stats.data;
  return <section className="view"><header className="view-header"><div><button className="text-button" type="button" onClick={onBack}>{t("back")}</button><h1>{details.data?.referral_code || t("referrals")}</h1></div><SegmentedControl options={["day", "week", "month"]} value={period} onChange={setPeriod} ariaLabel={t("filters")} t={t} /></header><Status {...stats} empty={false} t={t}>{data && <><div className="metrics"><article><span>{t("attachedUsers")}</span><strong>{data.attached_users}</strong></article><article><span>{t("earned")}</span><strong>{formatMoney(data.earned_cents, locale)}</strong></article><article><span>{t("available")}</span><strong>{formatMoney(data.available_cents, locale)}</strong></article><article><span>{t("pendingWithdrawals")}</span><strong>{formatMoney(data.reserved_cents, locale)}</strong></article><article><span>{t("paid")}</span><strong>{formatMoney(data.paid_cents, locale)}</strong></article></div><section className="revenue-panel"><header><div><h2>{t("referralIncome")}</h2><strong>{formatMoney(data.earned_cents, locale)}</strong></div></header><RevenueChart series={data.income_series} locale={locale} t={t} /></section></>}</Status><section className="data-section"><header><h2>{t("referralPurchases")}</h2></header><Status {...purchases} empty={!purchases.data?.items?.length} t={t}><div className="table-scroll"><table><thead><tr><th>{t("username")}</th><th>{t("purchased")}</th><th>{t("plan")}</th><th>{t("customerPayment")}</th><th>{t("referralCommission")}</th><th>{t("currency")}</th><th>{t("status")}</th></tr></thead><tbody>{purchases.data?.items.map((item, index) => <tr key={`${item.username}-${index}`}><td>{item.username || t("unknown")}</td><td>{formatDate(item.purchased_at, locale)}</td><td>{item.plan}</td><td>{formatMoney(item.amount_cents, locale)}</td><td>{formatMoney(item.commission_cents, locale)}</td><td>{item.currency}</td><td>{item.status}</td></tr>)}</tbody></table></div><Pagination meta={purchases.data?.meta} onPage={setPage} t={t} /></Status></section></section>;
}
...
function Application({ profile, onSignedOut, locale, theme, onThemeChange, localePreference, onLocaleChange, t }) {
  ...
  const navigation = [["dashboard", "dashboard"], ["administrators", "administrators"], ["paid-users", "paidUsers"], ["transactions", "transactions"], ["reports", "reports"], ["referrals", "referrals"]];
  useEffect(() => {
