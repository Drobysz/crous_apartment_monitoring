...
function Referrals({ t, locale }) {
  ...
  return <section className="view"><header className="view-header"><div><h1>{t("referrals")}</h1><p>{remote.data?.items?.length ?? 0} {t("results")}</p></div><button className="button button-primary" type="button" onClick={() => setAdding(true)}>{t("createReferral")}</button></header>{notice && <p className="field-help" role="status">{notice}</p>}<Status {...remote} empty={!remote.data?.items?.length} t={t}><div className="table-scroll"><table><thead><tr><th>{t("referralCode")}</th><th>{t("ownerUsername")}</th><th>{t("status")}</th><th>{t("available")}</th><th>{t("earned")}</th><th>{t("paid")}</th><th><span className="sr-only">{t("copyLink")}</span></th></tr></thead><tbody>{remote.data?.items.map((item) => <tr key={item.id}><td dir="ltr">{item.referral_code}</td><td dir="ltr">{item.owner_telegram_username}</td><td><button className="text-button" type="button" onClick={() => toggle(item)}>{item.is_active ? t("active") : t("inactive")}</button></td><td>{formatMoney(item.available_cents, locale)}</td><td>{formatMoney(item.lifetime_earned_cents, locale)}</td><td>{formatMoney(item.paid_cents, locale)}</td><td><button className="row-action" type="button" onClick={(event) => copy(item, event)}>{t("copyLink")}</button></td></tr>)}</tbody></table></div></Status>{adding && <ReferralForm onClose={() => setAdding(false)} onCreated={remote.reload} t={t} />}</section>;
}
